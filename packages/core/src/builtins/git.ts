...
import type { ToolDefinition, ToolRuntimeMetadata } from "../contracts/tools";
...
export type BuiltInGitOptions = Omit<GitPluginOptions, "pluginId">;
...
export function createGitPlugin(options: GitPluginOptions): LocalPlugin {
  ...
  return {
    ...
    name: "Guga Git Tools",
    init(context) {
      for (const tool of createBuiltInGitTools({ workspaceRoot: options.workspaceRoot, backend })) {
...
export function createBuiltInGitTools(options: BuiltInGitOptions): ToolDefinition[] {
  return gitTools(options.workspaceRoot, options.backend ?? createLocalGitBackend());
}
