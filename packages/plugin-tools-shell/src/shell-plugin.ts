...
import type { LocalPlugin, ToolDefinition } from "@guga-agent/core";
...
export function createShellPlugin(options: ShellPluginOptions): LocalPlugin {
  ...
  return {
    ...
    init(context) {
      context.registerTool(shellTool(options, backend, timeoutMs));
    }
  };
}
...
function shellTool(options: ShellPluginOptions, backend: ShellBackend, timeoutMs: number): ToolDefinition {
  return {
    ...
    runtime: {
      ...
      source: { kind: "first-party", packageName: "@guga-agent/plugin-tools-shell" },
