...
describe("delegation tool dependency boundary", () => {
  it("keeps delegation execution out of core", async () => {
    ...
    expect(corePackage.dependencies ?? {}).not.toHaveProperty("@guga-agent/plugin-tools-delegation");
    expect(corePackage.dependencies ?? {}).not.toHaveProperty("langgraph");
    expect(corePackage.dependencies ?? {}).not.toHaveProperty("crewai");
  });
});
