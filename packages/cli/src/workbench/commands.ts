...
export const WORKBENCH_SLASH_COMMAND_METADATA = [
  ...
  slashCommand({ command: "/login", label: "Login", usage: "/login <provider>", help: "Login to a provider.", selector: "provider" }),
  slashCommand({ command: "/logout", label: "Logout", usage: "/logout <provider>", help: "Remove a local provider credential." }),
  slashCommand({ command: "/auth", label: "Auth status", usage: "/auth status [provider]", help: "Show provider auth status." }),
