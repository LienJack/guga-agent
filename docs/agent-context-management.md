...
`pi agent` 补充了一个更贴近 Guga 新 roadmap 的边界：context/resource 不应只由 core 自己扫描，而可以由 `ResourceLoader` 和 extension events 共同提供。`createAgentSession()` 默认通过 `DefaultResourceLoader` 供应 extensions、skills、prompt templates、themes 和 context files，证据在 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/repomix/pi-focused-context.xml:37718` 到 `:37731`；`resources_discover` 事件允许 extension 增加 skill/prompt/theme paths，`session_before_compact` 可以取消或定制压缩，见同文件 `:32414`、`:32483`。这给 Guga 一个迁移判断：P0/P1 的 context 事实源可以先简单，但接口要允许插件贡献资源和定制 compaction。
...
| L4 事件账本与会话恢复 | 重启后能重建上下文，并知道何时压缩过 | `blade-code` `ContextAssembler`；`opencode` `message-v2`；`cc-haha` `compact_boundary`；`hermes-agent` SessionDB/session log；`pi` `AgentSessionRuntime` |
...
`pi agent` 的 `AgentSessionRuntime` 对恢复和会话替换的边界尤其有价值。它把 new session、resume、fork、switch session 这类“替换当前运行时”的 API 放在 `AgentSessionRuntime`，而不是普通 `AgentSession` 上；文档明确说 replacement APIs live on `AgentSessionRuntime`，并且 runtime factory 会按新的 cwd/session target 重新创建 cwd-bound services，证据在 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/repomix/pi-focused-context.xml:37786` 到 `:37825`。Guga 可以借这个边界：session resume/fork 不是简单替换 messages，而是要重新绑定 resource loader、extensions、tools、model registry 和 session manager。
