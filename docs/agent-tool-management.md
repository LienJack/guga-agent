...
- `/Users/lienli/Documents/GitHub/guga-agent/docs/research/repomix/pi-focused-context.xml`

## Guga 的取舍校准：权限元数据要早，插件生态要晚

Tool 管理和 ReAct loop 是同一条主线的两面。对 Guga 来说，P0 不应该继续手写 prompt 里的工具说明；最小 `ToolDefinition` 应该尽早包含 `effect`、`readonly`、`concurrencySafe`、`resultBudget` 和 permission 默认值。但这不意味着第一版就做插件生态、MCP marketplace 或复杂 artifact store。

- **P0 先收回声明权和执行权**：工具 schema、描述、execute、结果格式来自同一份 `ToolDefinition`；所有工具都经过最小 `ExecutionPipeline`。
- **P1 建权限和结果预算**：allow/ask/deny、路径安全、超时、abort、超大输出 eviction 是商业底线，不应等插件化后再补。
- **P1/P2 再做并发和 artifact**：并发必须基于 `readonly + concurrencySafe + resource/path`，artifact 是 runtime 产物，不是前端字符串。
- **P2 才开放插件/MCP/deferred tools**：外部工具只能作为 ToolRegistry 来源之一，不能绕过权限、调度和结果预算。

证据强度：ToolRegistry、ExecutionPipeline、权限三态和工具结果预算是多项目 `Fact`；Guga 把 effect/permission 元数据提到 P0 是 `Inference`；文件锁策略、artifact 存储形态和插件开放范围是 `Pending Verification`。
...
- `pi agent` 的 `pi.registerTool()` 是 extension-first 工具注册范本。`ToolDefinition` 不只有 name、description、parameters、execute，还包含 `promptSnippet`、`promptGuidelines`、`prepareArguments`、`executionMode`、`renderCall`、`renderResult`，证据在 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/repomix/pi-focused-context.xml:49940` 到 `:50380`。文档还说明 `registerTool()` 可以在 extension load、`session_start`、command handler 或其他 event handler 中调用，新工具会立刻刷新并可被 LLM 调用，见同文件 `:33292` 到 `:33304`。这给 Guga 的判断是：动态工具可以很早设计接口，但执行仍必须穿过 registry、permission 和 result normalization。
...
- `pi agent` 补足了工具结果展示和插件工具来源这条线。它允许工具定义自带 custom renderer，extension 也可以注册 message renderer；工具列表既可以来自 `createAgentSession({ customTools })`，也可以来自 extensions 的 `pi.registerTool()`，并带 source metadata，证据在 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/repomix/pi-focused-context.xml:33581`、`:33737`、`:33784`。Guga 可以借这个形态：tool result 的 UI 展示应是 renderer/plugin 层能力，但原始执行结果、artifact 引用和审计记录仍由 core/tool runtime 统一保存。
...
| 优先级 | 先做什么 | 暂时不做什么 |
| --- | --- | --- |
| P0 | `ToolDefinition`、ToolRegistry、顺序 ExecutionPipeline、tool_call/result 配对、effect/readonly/concurrencySafe/resultBudget 字段 | 任意 shell、插件系统、复杂并发 |
| P1 | allow/ask/deny、路径安全、timeout/abort、result truncation/eviction、tool state event、session approval | permanent allowlist、artifact 权限矩阵 |
| P2 | readonly 并发、路径锁、artifact store、MCP/custom/deferred tools、plugin adapter、toolset/profile policy | 无边界 marketplace、swarm 共享工具池 |
