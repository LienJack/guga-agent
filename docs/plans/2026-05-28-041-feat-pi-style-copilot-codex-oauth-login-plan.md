...
depends_on:
  ...
  - docs/plans/2026-05-28-042-feat-ink-tui-workbench-plan.md
deepened: 2026-05-28
...
本计划在已落地的 M40 provider/auth/model registry 基础上，新增 Guga-owned OAuth session 层，让 GitHub Copilot 与 OpenAI/Codex 可以像 Pi 一样从 Guga 登录、存储、刷新、登出、进入模型可用性视图，并最终通过 Guga runtime 发起真实调用。M42 已把交互入口迁到 Ink workbench；M41 的 workbench 登录体验应接入现有 slash palette、selector overlay 和 controller，而不是另建旧式 REPL 流程。

---

## Problem Frame

M40 已经解决多 provider 配置、API-key/env/local credential、模型选择和 AI SDK bridge 的基础，但仍把 OAuth 登录留到后续。M41 要补齐已有 Copilot 或 Codex 账号用户最在意的闭环：不是把 token 存起来就结束，而是登录后模型立刻变得可理解、可选择、可运行。

本计划保留 Guga 的 runtime authority：OAuth session、redaction 和 login UX 属于 CLI/host 层；模型可用性属于现有 `model-registry.ts` resolved model view；tool execution、router events、fallback 和 provider error normalization 仍由 Guga runtime 拥有。
...
- OAuth-specific workbench polish：本计划会让 `/login`、`/logout`、`/models` 接入 M42 的 slash/selector/controller surfaces；更完整的交互式登录弹窗和通知样式可在后续增强。
...
### Workbench UI integration note

OAuth/device-code 登录比 M40 static/env credential 更依赖交互面。M42 已经落地 Ink workbench、slash palette、selector overlay、controller 和焦点状态；M41 不应再把旧 line REPL 当主要产品面。当前 `/login` 仍只是 provider guidance，`guga login` 仍是 API-key/env/local credential setup；OAuth 实施需要把这些入口升级为真实 login/logout/auth status flow，并保持 headless `guga run`、`guga -p`、`--list-models`、`init` 的 Ink import boundary 不变。
...
- `packages/cli/src/guga-home.ts` 已提供 Guga Home、project key、cache/log/profile/session roots；M40 的 `provider-login.ts` 已把 local credential 写到 `credentials/providers/<provider>.json` reference 下，但 `GugaHomePaths` 还没有 typed `credentialsRoot`，M41 应补齐这个正式路径而不是另起 home 目录。
- `packages/cli/src/config.ts` 已提供 TOML-first layered config、named providers、model aliases、fallback models、`credentialRef` 和 env override；M41 应扩展 auth source，而不是让 OAuth 写入裸 API key 字段。
- `packages/cli/src/provider-auth.ts` 已解析 env/static/local API key 并返回 redacted auth view，状态目前是 `configured | missing | invalid | unknown`；M41 应扩展成 OAuth-aware session state，而不是新增并行 auth resolver。
- `packages/cli/src/provider-login.ts` 已实现 API-key/env/local credential 的 `guga login` 写入路径；M41 应复用或重构这个入口，使 `guga login copilot/codex` 能根据 provider 类型进入 OAuth flow。
- `packages/cli/src/model-registry.ts` 已是 M40 的 resolved model availability view，合并 config、registered model metadata、auth、health 和 capability；M41 应在这里增加 OAuth-backed auth 状态与 built-in Copilot/Codex aliases，而不是创建第二套 model availability source。
- `packages/cli/src/commands/run.ts` 是 `guga init`、`guga login`、`guga run`、interactive 和 `--list-models` 的入口；`guga logout/auth status` 应从这里进入，但实现逻辑应留在独立 auth 模块中。
- `packages/cli/src/workbench/commands.ts` 与 `packages/cli/src/workbench/model-control.ts` 已承载 `/login`、`/models` 和 `/model`；M41 应复用同一个 model registry view 显示 missing auth、configured、expired 等状态。
- `packages/cli/src/ink-workbench/controller.ts`、`selector-state.ts` 和 `app.tsx` 已承载 M42 的 slash palette、selector overlay、focus restoration 和 command execution；M41 的 Pi-style `/login` 应接入这些 renderer-neutral/controller surfaces。
- `packages/cli/src/host-factory.ts` 已把 resolved model registry 转成 provider plugins 与 router policy；M41 应在这里解析 OAuth credential 并把 redacted provider config 交给 provider plugin。
...
| Auth state 并入现有 `model-registry.ts` resolved model view | 用户看到的是模型是否可选、为何不可选；独立 OAuth 状态页不足以满足 R10-R12。 |
  ...
  cli/
    src/
      ...
      ink-workbench/
        ...
        controller.test.ts
        selector-state.ts
        selector-state.test.ts
        app.test.tsx
        workbench-smoke.test.tsx
      guga-home.ts
      guga-home.test.ts
      host-factory.ts
...
**Dependencies:** M40 complete, M42 complete for workbench selector/controller paths
...
- Modify: `packages/cli/src/provider-login.ts`
- Create or Modify: `packages/cli/src/provider-login.test.ts`
- Create: `packages/cli/src/provider-credential-store.ts`
- Create: `packages/cli/src/provider-credential-store.test.ts`

**Approach:**
- Extend Guga Home diagnostics with a typed credentials root such as `credentialsRoot`, aligning with the existing `credentials/providers/<provider>.json` convention from `provider-login.ts` and preserving project partitioning only where needed for status display.
...
- `packages/cli/src/provider-auth.ts`
- `packages/cli/src/provider-login.ts`
...
- Add slash commands with the same state changes as CLI commands. Workbench command handlers should return display-safe messages and structured data for the Ink controller and future richer UI.
- In the real workbench path, `/login` must be discoverable from the slash popover and provider choice must use existing selector/confirm overlays. The direct `/login <provider>` path remains a power-user fallback, but should no longer be the only product path.
...
- `packages/cli/src/ink-workbench/controller.ts`
- `packages/cli/src/ink-workbench/selector-state.ts`
...
**Goal:** Make Copilot/Codex models appear in the same existing `model-registry.ts` resolved model availability view as other providers, with unavailable reasons before login and selectable models after login.
...
**Files:**
- Modify: `packages/cli/src/model-registry.ts`
- Modify: `packages/cli/src/model-registry.test.ts`
...
**Approach:**
- Extend the existing `model-registry.ts` view with OAuth-aware availability inputs: provider id, provider mode, model id, auth source, auth status, unavailable reason, and safe account hints.
...
**Patterns to follow:**
- `packages/cli/src/model-registry.ts`
- `packages/cli/src/provider-auth.ts`
- `packages/cli/src/config.ts`
- `packages/cli/src/workbench/model-control.ts`
...
| Existing API-key `guga login` semantics conflict with OAuth login expectations | U3/U4 should make provider type explicit: API-key providers keep current flags, OAuth-backed Copilot/Codex enter flow runners without requiring `--api-key`. |
| Ink workbench selector/controller path diverges from slash command parser | U4 should test both `executeWorkbenchCommand()` and `WorkbenchController.selectorForCommand()` so typed slash fallback and palette-driven selection stay aligned. |
...
- **Depends on:** [docs/plans/2026-05-28-042-feat-ink-tui-workbench-plan.md](2026-05-28-042-feat-ink-tui-workbench-plan.md)
- Related code: `packages/cli/src/guga-home.ts`
- Related code: `packages/cli/src/config.ts`
- Related code: `packages/cli/src/provider-auth.ts`
- Related code: `packages/cli/src/provider-login.ts`
- Related code: `packages/cli/src/model-registry.ts`
- Related code: `packages/cli/src/commands/run.ts`
- Related code: `packages/cli/src/workbench/commands.ts`
- Related code: `packages/cli/src/ink-workbench/controller.ts`
- Related code: `packages/cli/src/ink-workbench/selector-state.ts`
- Related plan: `docs/plans/2026-05-28-037-feat-productized-cli-workbench-plan.md` for the original prompt editor, slash popover, selector, and focus-stack direction.
- Related plan: `docs/plans/2026-05-28-042-feat-ink-tui-workbench-plan.md` for the completed Ink workbench entry point and selector/controller surfaces.
...
- Related learning: `docs/solutions/architecture-patterns/multi-provider-login-switching.md`
