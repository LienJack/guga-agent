...
| M4 Context Policy / Attention OS Projection | 长任务中模型每轮看到的内容如何从消息历史升级为可解释、可预算、可压缩、可审计的 Attention OS projection。 | Context compression pack, context-policy research docs, Guga M3 result policy | [M4 plan research section](../plans/2026-05-27-001-feat-context-policy-plugins-plan.md#context--research) | Model input projection, typed facts/state/trace/memory-candidate descriptors, compaction continuity, reinjection, replay/audit metadata, and default context policy plugin. |
...
| Web Search Capability | 外部互联网搜索如何成为可选 first-party extension，同时保留权限、availability、预算和审计。 | Guga tool registry/runtime patterns, OpenCode, Claude Code, Pi, Brave Search API | [web-search-capability](../solutions/architecture-patterns/web-search-capability.md) | `@guga-agent/plugin-web-search` contributes `web_search` through the extension SDK with mock and Brave backends, default-disabled CLI config, and runtime pipeline integration. |
...
| M26 Memory JSONL Retrieval | durable memory JSONL 如何直接做 scope-required retrieval，同时保留 JSONL diagnostics。 | Guga M17 JSONL store, M18 scoped retrieval | [memory-jsonl-retrieval.md](./memory-jsonl-retrieval.md) | `@guga-agent/plugin-memory-jsonl` adds `readRetrieval()` to search governed durable records with canonical retrieval semantics. |
| M27 Memory JSONL Curated Markdown | durable memory JSONL 如何直接提供 active-safe curated Markdown，同时保留 JSONL diagnostics。 | Guga M17 JSONL store, M19 memory Markdown export | [memory-jsonl-curated-markdown.md](./memory-jsonl-curated-markdown.md) | `@guga-agent/plugin-memory-jsonl` adds `readCuratedMarkdown()` to render durable governed memory with canonical Markdown export semantics. |
| M28 Memory JSONL Capability Surface | durable JSONL memory 的 review / retrieval / curated Markdown projections 如何进入 capability discovery。 | Guga M21 memory review capability, M22-M27 JSONL read projections | [memory-jsonl-capability-surface.md](./memory-jsonl-capability-surface.md) | `@guga-agent/plugin-memory-jsonl` registers read-only projection operation descriptors while preserving the broad `memory.jsonl` descriptor. |
| M29 Memory JSONL Audit Snapshot | durable memory JSONL 如何一次读取 ledger / report / health / Markdown audit bundle。 | Guga M22 review report, M23 review Markdown, M25 review health | [memory-jsonl-audit-snapshot.md](./memory-jsonl-audit-snapshot.md) | `@guga-agent/plugin-memory-jsonl` adds `readAuditSnapshot()` to build the durable audit bundle from one governance ledger read. |
| M30 Memory JSONL Health Markdown | durable memory JSONL 如何提供紧凑 review health Markdown block。 | Guga M24 review health, M25 JSONL review health | [memory-jsonl-health-markdown.md](./memory-jsonl-health-markdown.md) | `@guga-agent/plugin-memory-jsonl` adds `readReviewHealthMarkdown()` to render durable health with canonical health block semantics. |
| M31 Memory JSONL Health Capability | durable JSONL health projection 如何进入 capability discovery，而不扩大写入权限。 | Guga M28 capability surface, M30 health Markdown | [memory-jsonl-health-capability.md](./memory-jsonl-health-capability.md) | `@guga-agent/plugin-memory-jsonl` registers `memory.jsonl.health` as a first-party read-only operation descriptor. |
| M32 Memory JSONL Audit Snapshot Capability | durable JSONL audit bundle 如何进入 capability discovery，而不变成写入或执行能力。 | Guga M28 capability surface, M29 audit snapshot | [memory-jsonl-audit-snapshot-capability.md](./memory-jsonl-audit-snapshot-capability.md) | `@guga-agent/plugin-memory-jsonl` registers `memory.jsonl.audit_snapshot` as a first-party read-only operation descriptor. |
| M33 Memory JSONL Review Markdown Capability | durable JSONL review Markdown 如何进入 capability discovery，同时保留 broad review descriptor。 | Guga M23 review Markdown, M28 capability surface | [memory-jsonl-review-markdown-capability.md](./memory-jsonl-review-markdown-capability.md) | `@guga-agent/plugin-memory-jsonl` registers `memory.jsonl.review_markdown` as a first-party read-only operation descriptor. |
| M34 Memory JSONL Review Report Capability | durable JSONL typed review report 如何进入 capability discovery，同时保留 broad review descriptor。 | Guga M22 review report, M28 capability surface | [memory-jsonl-review-report-capability.md](./memory-jsonl-review-report-capability.md) | `@guga-agent/plugin-memory-jsonl` registers `memory.jsonl.review_report` as a first-party read-only operation descriptor. |
| M35 Memory JSONL Capability Name Constants | JSONL memory capability 名称如何从散落字符串变成稳定可导出的 host-facing vocabulary。 | Guga M28-M34 capability surface | [memory-jsonl-capability-name-constants.md](./memory-jsonl-capability-name-constants.md) | `@guga-agent/plugin-memory-jsonl` exports readonly operation-name constants and uses them during descriptor registration. |
| M36 Memory JSONL Capability Namespace | JSONL memory capability family 如何通过标准 namespace metadata 被 host 过滤和解释。 | Guga capability discovery namespace contract, M35 name constants | [memory-jsonl-capability-namespace.md](./memory-jsonl-capability-namespace.md) | `@guga-agent/plugin-memory-jsonl` exports `MEMORY_JSONL_OPERATION_NAMESPACE` and adds it to every operation descriptor. |
| M37 Memory JSONL Public Capability Exports | JSONL memory capability constants 如何通过 package entrypoint 被 host 可靠导入。 | Guga public export discipline, M35/M36 constants | [memory-jsonl-public-capability-exports.md](./memory-jsonl-public-capability-exports.md) | `@guga-agent/plugin-memory-jsonl` adds a public entrypoint test for operation-name and namespace constants. |
