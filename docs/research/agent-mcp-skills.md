...
本文沿用 `docs/research/agent-react-pattern.md` 的写法：不做目录导览，而是给一条可以复刻的阶段路线图。参考项目版本锚点来自 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/intake/source-contract.md`：`blade-agent-sdk@5d67e5e`、`blade-code@ad67f3d`、`opencode@caf1151`、`deepagentsjs@7c33a86`、`deer-flow@84f88b6`、`cc-haha@dbb8c95`、`hermes-agent@dd0923b`、`pi`。
...
`pi agent` 最适合补充“资源发现和插件包”这一层。`DefaultResourceLoader` 会发现 extensions、skills、prompt templates、themes 和 context files，`createAgentSession()` 如果不传 custom loader 就使用这套默认发现，证据在 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/repomix/pi-focused-context.xml:37718` 到 `:37731`、`:38493`；extension 还能通过 `resources_discover` 增加 skill/prompt/theme 路径，见同文件 `:32414` 到 `:32431`。pi packages 还把 extensions、skills、prompts、themes 打包分享，见同文件 `:35530` 之后。Guga 可以借这个方向：MCP/skill/plugin 不应是三套互不相干的入口，而应统一进 ResourceLoader/CapabilityRegistry。
...
第四个可借设计点是 `resource discovery as plugin surface`。`pi agent` 通过 `DefaultResourceLoader` 和 `resources_discover` 让 extensions、skills、prompts、themes、context files 进入统一发现链；这比每个子系统各扫一遍目录更容易支持 reload、package、project/user precedence 和审计。

适用场景：你希望把 MCP、skills、prompt templates、themes 和插件包统一成一个 capability/resource discovery 面。

不适用场景：只有固定内置工具和少量项目规则文件。此时普通配置加载比 ResourceLoader 更简单。

迁移优先验证：reload 后路径是否去重、project/user precedence 是否稳定、插件贡献资源是否可审计、禁用插件后资源是否消失。

第五个可借设计点是 `dynamic registry generation`。`hermes-agent` 的 generation counter 和 lock 不是形式主义。只要 MCP tools 会动态刷新、plugin 会热加载、skill 会更新，prompt/tool definition cache 就必须知道什么时候失效。
