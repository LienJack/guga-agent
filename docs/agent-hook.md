...
- Inference: Blade Agent SDK / Blade Code 的 `ExecutionPipeline` 与 `runToolCall` 适合作为 Guga tool execution skeleton；现有 Guga 研究文档已把它归纳为 parse/repair/pre-hook/confirmation/execute/post-hook/history，见 `/Users/lienli/Documents/GitHub/guga-agent/docs/agent-tool-management.md`。
