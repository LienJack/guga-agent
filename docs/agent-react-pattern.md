...
本文沿用 `arch-insight` 的 `Article - Deep Dive` 交付模式：不是源码目录导览，而是用真实源码路径建立设计判断。参考项目版本锚点来自 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/intake/source-contract.md`：`blade-agent-sdk@5d67e5e`、`blade-code@ad67f3d`、`opencode@caf1151`、`deepagentsjs@7c33a86`、`deer-flow@84f88b6`、`cc-haha@dbb8c95`、`hermes-agent@dd0923b`、`claude-code@3d7b32f`、`pi`。Claude Code 的证据优先引用本仓库 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/source-analysis/claude-code-analysis/` 与 focused repomix context，必要时再回到 `/Users/lienli/Documents/GitHub/agent-ref/claude-code/src/` 做行级确认。`cc-haha` 只作为远端事件、权限、compact boundary 的桥接参考，不作为 ReAct core 参考；`hermes-agent` 也不适合当 L0/L1 范本，它更像 L5 产品态 runtime 的压力样本；`pi agent` 则适合作为 extension-first session runtime 参考，不应被误读成纯 ReAct loop 范本。
...
`pi agent` 值得放在“可编程 session runtime”位置。它的 `createAgentSession()` 是单 session 工厂，默认用 `DefaultResourceLoader` 发现 extensions、skills、prompt templates、themes 和 context files；`AgentSessionRuntime` 则拥有 new/resume/fork/switch 这类替换当前 runtime 的 API，证据在 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/repomix/pi-focused-context.xml:37718` 到 `:37825`。这说明 Guga 如果走 core + plugin 路线，ReAct loop 不只是 while 状态机，还要有 session runtime 负责绑定 cwd、resource loader、model registry、extensions 和 session manager。
...
## Guga 的取舍校准：路线分层，不等于能力都延后

上面的 L0 到 L5 是复杂度教学顺序，不完全等于 Guga 的工程优先级。结合 `agent-loop`、`tool-registry`、`context-compression`、`provider-abstraction`、`ui-protocol` 和 `multi-agent` 六个 context pack，有几条需要提前钉住的本地判断：

- **Provider 边界要早，Provider 矩阵要晚**：L0 不需要一次接 20 个模型，但内部消息格式、tool schema、usage 和 stream 事件必须先归一。`provider-abstraction.md` 给出的共识是内部用 OpenAI 兼容格式，Anthropic/Bedrock/OpenAI Responses 等差异放到 Transport 层。
- **Streaming 可以先作为内部健康路径，不必先做 UI 流式体验**：`agent-loop.md` 把 Hermes 的 Always Streaming 列为 P0，原因是流式路径更容易做 stale-stream 检测和超时控制。因此 Guga 可以在 L0/L2 就让模型适配器走 streaming-first，再到 L3 才把事件完整投影给 UI。
- **权限不是 L5 才出现，组织级权限才是 L5**：最小 ToolRegistry 在 L1/L2 就应记录 `effect`、`readonly`、`concurrencySafe` 和 `allow/ask/deny` 决策结果；L5 再扩展到组织策略、远端授权和跨客户端 pending permission。
- **持久化不是“有空再补”的外设**：L4 才完整恢复长任务，但从第一版 loop 起就应该以 append-only JSONL 或等价 event log 写下 run/turn/tool/result。`context-compression.md` 和 Claude Code 都显示：写入层越朴素，恢复层越有空间修复链路。
- **Server/SSE 是产品入口，不应污染 core loop**：`ui-protocol.md` 建议 Guga 以 HTTP Server + SSE 作为 Phase 1 能力。更稳的边界是：L3 定义 core runtime event，Server/SSE 只是 `EventProjector` 的第一个投影，而不是把 HTTP 状态塞回 loop。
- **Multi-agent 先做单层委派，别先做 swarm**：`multi-agent.md` 的 Guga 迁移判断偏向 DeerFlow/Hermes：一个自包含 `delegateTask(goal, context, toolsets)`、单层、带独立预算和 trace id。Claude Code swarm/team/mailbox 属于 L5 后期，不应进入 ReAct core MVP。

这些判断的证据强度可以这样读：Provider/Tool/Event/JSONL 边界是多项目交叉 `Fact`；Guga 分期是基于现有 roadmap 与 context pack 的 `Inference`；具体阈值，例如 Hermes 50% 压缩触发、工具输出 50KB/100KB 限制，仍是 `Pending Verification`，需要等 Guga 的目标模型和真实任务轨迹确定后再定。
...
- 固定内部消息格式和 provider adapter 边界；可以只接一个 provider，但不要让主 loop 直接依赖某个 SDK 的原始事件。
...
- 不要先抽象多 provider 矩阵。一个 provider 跑通即可，但内部协议要先稳定。
...
- 支持模型返回 `tool_calls`。
- 定义最小 `ToolDefinition`：`name`、`description`、`inputSchema`、`effect`、`readonly`、`concurrencySafe`。
...
- 不要做并发工具执行。先保证顺序和配对完全正确，但并发安全元数据可以先存在 schema 里。
...
- 增加 `maxTurns` 和硬安全上限。
- 引入 `IterationBudget` 或等价预算对象，区分普通 turn、可退还的轻量工具调用和子 agent 独立预算。
...
- Server/SSE、CLI、Web、IDE 都只消费 runtime event 的投影，不直接读取 provider stream。
...
- 保存 append-only JSONL 或等价 event log：用户消息、assistant 消息、tool use、tool result、compaction 事件都可恢复。
- 给大工具结果加三层防护：工具内预截断、单结果落盘预览、单轮聚合预算。
...
- 插件化运行时：tools、providers、prompt modifiers、resource discovery、UI renderers 可以通过插件注册，但仍由 runtime 统一治理。
- 单层委派：通过 `delegateTask(goal, context, toolsets, traceId)` 创建上下文隔离的子 agent；父子预算、权限和 trace 必须分开。
...
- `/Users/lienli/Documents/GitHub/guga-agent/docs/research/repomix/pi-focused-context.xml:37718` 到 `:37825` 展示 `createAgentSession()`、`ResourceLoader` 和 `AgentSessionRuntime` 如何把 session lifecycle、resource discovery 和 runtime replacement 分层；`:32075` 到 `:32175` 展示 extension 可以注册 tools、commands、UI 和事件 handler。
...
- 子 agent 只能通过父级授权的工具交集执行任务，不能递归无限委派。
...
- 不要先实现 Claude Code swarm/team/mailbox。Guga 初期更适合单层委派和任务状态机。
...
6. L5 再做组织级权限、复杂并发工具计划、多客户端投影、观测、回放和远端桥接；这时再参考 Claude Code 的 Tool pipeline、append-only transcript、swarm/teammate 侧链，以及 `hermes-agent` 的 `AIAgent.run_conversation`、approval queue、gateway/API/ACP，而不是把这些一开始塞进最小 loop。

如果面向 Guga 当前阶段，可以把上面的路线压成更具体的优先级：

| 优先级 | 先做什么 | 暂时不做什么 |
| --- | --- | --- |
| P0 | 内部 OpenAI 消息格式、单 provider Transport、顺序 tool loop、tool_call 配对、abort/maxTurns、append-only JSONL、最小 SSE event | 多 provider 矩阵、复杂权限 UI、长记忆、swarm |
| P1 | streaming-first adapter、tool state、allow/ask/deny、readonly 并发、安全路径冲突检测、主动/被动 compact、大工具结果落盘、session resume | 完整组织策略、FTS5 历史搜索、ACP/LSP |
| P2 | 单层 `delegateTask`、子 agent 独立预算和 trace、HTTP Server + SDK、跨客户端权限投影、观测与 replay | Claude Code 式 coordinator/swarm、Hermes 式多平台网关、MoA |
