...
## Guga 的取舍校准：Provider 边界要早，Provider 平台要晚

LLM 接入的阶段路线不应该被理解成“先随便 call 一个 SDK，以后再抽象”。Guga 的 P0 就要把 provider SDK 关在边界里，但不需要一开始做完整 provider marketplace。`provider-abstraction.md` 的迁移判断很清楚：内部统一消息格式、Transport 分层、主/辅模型分离、prompt caching 是高 ROI；凭证池、models.dev、动态插件是后续运营能力。

- **P0 固定内部 OpenAI 兼容消息格式**：agent loop、tool executor、context manager 都只认识 Guga 自己的消息和事件类型。
- **P0 只接一个真实 provider，但通过 Transport 接**：不要为了快把 SDK 类型泄漏到 loop；也不要第一天接 20 个 provider。
- **P1 引入 streaming-first 和错误分类**：streaming 是健康检查和取消传播基础；context overflow、rate limit、auth、abort 要成为 loop decision 的输入。
- **P2 再做 provider 运营平台**：models.dev、credential pool、用户自带 key、provider profile/plugin、成本仪表盘都等两个以上真实 provider 跑过后再扩展。
- **Provider plugin surface 先学 pi，不先学 marketplace**：`pi.registerProvider()` 让 extension 注册或注销自定义 provider，但 provider 仍进入 runtime 的 model registry；Guga 可以先做这个轻量插件面，再考虑 provider 市场。

证据强度：OpenAI 内部格式、Transport/adapter 边界、stream event adapter 是多项目 `Fact`；Guga P0 采用单 provider + Transport 是 `Inference`；最终是否用 Vercel AI SDK、Hermes 式自研 Transport 或混合模式是 `Pending Verification`。
...
`pi agent` 是 provider plugin surface 的轻量范本。extension 文档展示 `pi.registerProvider("my-provider", config)` 和 `pi.unregisterProvider(name)`，并说明 register/unregister 在初始加载后也能立即生效，证据在 `/Users/lienli/Documents/GitHub/guga-agent/docs/research/repomix/pi-focused-context.xml:31117`、`:33616`、`:33686`；loader 侧还会把 provider registrations queue 到 runtime/model registry，相关实现片段在同文件 `packages/coding-agent/src/core/extensions/loader.ts` 与 `runner.ts` 附近。Guga 的可借点不是照搬 pi 的 provider config 形状，而是把“自定义 provider”做成插件能力，同时仍由 core registry 负责冲突、启停和能力枚举。
...
| 优先级 | 先做什么 | 暂时不做什么 |
| --- | --- | --- |
| P0 | 单 provider Transport、内部消息/工具 schema 转换、normalized usage、AbortSignal、基础错误包装 | provider registry、模型市场、复杂 credential pool |
| P1 | streaming-first `LLMEvent`、capability 表、tool schema/provider 兼容、context overflow 分类、compaction retry 信号 | 动态 npm/provider import、企业网关、模型自动发现 |
| P2 | auxiliary model routing、prompt caching、models.dev 缓存、fallback chain、credential rotation、cost/usage dashboard | Hermes 式用户 provider plugin 覆盖体系 |
